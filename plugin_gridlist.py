# -*- coding: utf-8 -*-
"""
Advanced Arabic Player - Custom grid widgets
"""
import os
...
def resolve_icon_path(item, plugin_path=PLUGIN_PATH):
    ...
    if action.startswith("site_"):
        ...
        if os.path.exists(icon_file): icon_path = icon_file
    if not icon_path:
        default_icon = os.path.join(plugin_path, "plugin.png")
        if os.path.exists(default_icon): icon_path = default_icon
    return icon_path
...
class _BaseCardGrid(GUIComponent):
    ...
    def __init__(self, cols, rows, cell_w, cell_h, font_size=24):
        ...
        self.blink_state = False # For Grid blinking
...
class HomeMenuGrid(_BaseCardGrid):
    ...
    def _buildRow(self, row_idx):
        ...
        for col_idx in range(self.cols):
            ...
            row.append(MultiContentEntryText(pos=(cx + HOME_BORDER_W + 8, ty), size=(HOME_CELL_INNER_W - 2 * HOME_BORDER_W - 16, 40), font=0, text=title, color=_G_CLR["text"], backcolor=_G_CLR["surface2"], flags=RT_HALIGN_CENTER))
            if tagline:
                tag_y = ty + 44
                row.append(MultiContentEntryText(pos=(cx + HOME_BORDER_W + 8, tag_y), size=(HOME_CELL_INNER_W - 2 * HOME_BORDER_W - 16, 32), font=0, text=tagline, color=_G_CLR["text2"], backcolor=_G_CLR["surface2"], flags=RT_HALIGN_CENTER))
        return row

# --- 8x2 Movie/Series Grid ---
POSTER_GRID_COLS = 8
POSTER_GRID_ROWS = 2
POSTER_W = 210
POSTER_H = 340
POSTER_CAPTION_H = 30
POSTER_CELL_MARGIN_H = 10
POSTER_CELL_MARGIN_V = 20
...
class PosterCardGrid(_BaseCardGrid):
    ...
    def _buildRow(self, row_idx):
        ...
        for col_idx in range(self.cols):
            ...
            # 1. Blinking Border (Drawn underneath the poster)
            frame_color = _G_CLR["cyan"] if (is_sel and self.blink_state) else _G_CLR["border"]
            row.append(MultiContentEntryText(pos=(cx - 6, cy - 6), size=(POSTER_W + 12, POSTER_H + 12), font=0, text="", color=0, backcolor=frame_color, flags=0))
            
            # 2. Solid Pure Opaque Black Background for Poster (Fixes transparent backgrounds in some skins)
            row.append(MultiContentEntryText(pos=(cx, cy), size=(POSTER_W, POSTER_H), font=0, text="", color=0, backcolor="#FF000000", flags=0))
            
            # 3. Resume Badge (If item has saved position > 30s)
            saved_pos = _get_saved_position(item.get("url", ""))
            if saved_pos > 30:
                row.append(MultiContentEntryText(pos=(cx + POSTER_W - 80, cy + 5), size=(75, 26), font=1, text="RESUME", color=_G_CLR["gold"], backcolor="#FF000000", flags=RT_HALIGN_CENTER | RT_VALIGN_CENTER))
...
# --- XtreamNew UI Implant: Carousel XML Generators ---
_CAROUSEL_GEOMETRY = {
...
def build_carousel_xml():
    ...
    for i in range(7):
        ...
        parts.append(f'<widget name="cfocus{i}" position="0,0" size="1,1" backgroundColor="#00E5FF" cornerRadius="12" zPosition="3" transparent="0" />')
        parts.append(f'<widget name="cposter{i}" position="0,0" size="1,1" backgroundColor="#161B22" cornerRadius="12" zPosition="4" transparent="0" halign="center" valign="center" font="Regular;24" foregroundColor="#00E5FF" />')
        parts.append(f'<widget name="cposterImg{i}" position="0,0" size="1,1" zPosition="5" alphatest="blend" scale="1" />')
        parts.append(f'<widget name="cfavMark{i}" position="0,0" size="1,1" font="Regular;24" foregroundColor="#FFD740" transparent="1" zPosition="6" halign="center" valign="center" />')
        parts.append(f'<widget name="cratingBadge{i}" position="0,0" size="1,1" font="Regular;18" foregroundColor="#39D98A" backgroundColor="#000000" transparent="0" cornerRadius="8" zPosition="6" halign="center" valign="center" />')
        parts.append(f'<widget name="cresumeMark{i}" position="0,0" size="1,1" font="Regular;18" foregroundColor="#FFD740" backgroundColor="#000000" transparent="0" cornerRadius="8" zPosition="6" halign="center" valign="center" />')
    return "\n".join(parts)